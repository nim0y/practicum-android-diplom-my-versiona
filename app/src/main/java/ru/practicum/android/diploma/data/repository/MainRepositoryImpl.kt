...
class MainRepositoryImpl(private val networkClient: NetworkClient) : MainRepository {
    ...
    override suspend fun getSimilarVacancies(id: String): Response<out SearchResponseModel> {
        ...
        return if (response.resultCode == Constants.CODE_SUCCESS) Response.Success((response as SearchResponseDto).mapToModel())
        else Response.Error()
    }
    ...
    override suspend fun getCurrentVacancyDetails(id: String): Response<out VacancyModel> {
        ...
        return if (response.resultCode == Constants.CODE_SUCCESS) Response.Success((response as VacancyDto).mapToModel())
        else Response.Error()
    }

    override suspend fun getVacancies(query: String, page: Int, filters: HashMap<String, String>): Response<out SearchResponseModel> {
